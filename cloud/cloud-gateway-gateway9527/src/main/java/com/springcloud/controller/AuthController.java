...
import com.springcloud.annotation.JwtCheck;
import com.springcloud.dto.Result;
import com.springcloud.dto.UserDTO;
import com.springcloud.jwt.JwtModel;
...
public class AuthController {
    ...
    public Result<String> login(@RequestBody UserDTO userDTO) throws Exception {
        ...
        return new Result<String>(HttpStatus.SC_OK, "认证成功", jwt);
    }
    ...
    public Result<String> unauthorized() {
        return new Result<String>(HttpStatus.SC_UNAUTHORIZED, "未认证,请重新登陆", null);
    }
    ...
    @JwtCheck
    public Result<String> testJwtCheck(@RequestHeader("Authorization") String token, @RequestParam("name") @Valid String name) {

        return new Result<String>(HttpStatus.SC_OK, "请求成功咯", "请求成功咯" + name);
